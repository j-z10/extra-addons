...
from .helpers import Session, Record, WorkflowActivity
...
class WorkflowItem(object):
            ...
            if self.workitem['state'] == 'active':
                ...
                if activity.get('action', False):
                    ...
                    if not id_new:
                        ...
                        return False
                    assert type(id_new) == type(1), 'Wrong return value: ' + str(
    ...
    def _split_test(self, split_mode, signal, stack):
        ...
        if test and transitions:
            cr.executemany(
                'insert into wkf_witm_trans (trans_id,inst_id) values (%s,%s) except (select trans_id,inst_id from wkf_witm_trans)',
                transitions)
            cr.execute('delete from wkf_workitem where id=%s', (self.workitem['id'],))
